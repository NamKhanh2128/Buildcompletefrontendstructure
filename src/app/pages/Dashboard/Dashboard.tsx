...
const expiryAlerts = [
  { id: 1, name: 'Sữa tươi Vinamilk', daysLeft: 2, category: '🥛 Tủ lạnh', priority: 'high', emoji: '🥛', quantity: 2, unit: 'hộp', location: 'Tủ lạnh' },
  { id: 2, name: 'Thịt ba chỉ', daysLeft: 3, category: '🥩 Ngăn đông', priority: 'high', emoji: '🥩', quantity: 500, unit: 'gram', location: 'Ngăn đông' },
  { id: 3, name: 'Cà chua bi', daysLeft: 5, category: '🍅 Tủ lạnh', priority: 'medium', emoji: '🍅', quantity: 1, unit: 'kg', location: 'Tủ lạnh' },
  { id: 4, name: 'Dưa leo', daysLeft: 7, category: '🥒 Tủ lạnh', priority: 'low', emoji: '🥒', quantity: 3, unit: 'củ', location: 'Tủ lạnh' },
];

const initialTodayMeals = [
  { id: 1, time: 'Sáng • 7:00', dish: 'Phở bò Hà Nội', status: 'done', emoji: '🍜' },
  { id: 2, time: 'Trưa • 12:00', dish: 'Cơm gà xối mỡ', status: 'ready', emoji: '🍗' },
...
export function Dashboard() {
  ...
  const handleAddMeal = (data: any) => {
    const newMeal = {
      id: todayMeals.length + 1,
      time: `${data.mealType} • Mới thêm`,
      dish: data.recipeName || "Món mới",
      status: "pending",
      emoji: "🍽️",
    };
    setTodayMeals(prev => [...prev, newMeal]);
    success("Thêm bữa ăn thành công!", `Đã thêm "${newMeal.dish}" vào kế hoạch hôm nay.`);
  };
