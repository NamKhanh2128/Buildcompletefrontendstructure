...
import { Bell, Lock, User, Globe, Camera, Palette, Shield, ChevronRight, Eye, EyeOff, Check } from "lucide-react";
...
export function Settings() {
  ...
  const [profile, setProfile] = useState({
    name: "Nguyễn Văn A",
    email: "vana@nateat.com",
    phone: "0901234567",
    bio: "Yêu nấu ăn và chia sẻ bữa cơm gia đình ❤️",
  });
  ...
  const handleToggleNotification = (key: keyof typeof notifications) => {
    ...
    if (newValue) {
      success(`🔔 Bật thông báo`, `"${label[key]}" đã được bật.`);
    } else {
      info(`🔕 Tắt thông báo`, `"${label[key]}" đã được tắt.`);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-slide-up">
      ...
      <Card className="border-none shadow-[var(--shadow-card)] rounded-[var(--radius)]">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <p className="font-bold text-[var(--text-dark)]">{profile.name}</p>
              <p className="text-sm text-[var(--text-muted)] mt-0.5">Gia đình Nguyễn · Trưởng nhóm</p>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" onClick={() => setShowChangePassword(false)} className="rounded-[var(--radius-btn)] font-semibold">
              Hủy
            </Button>
            <Button onClick={handleChangePassword} className="bg-gradient-purple text-white rounded-[var(--radius-btn)] shadow-[var(--shadow-btn)] font-semibold hover-lift">
              Đổi mật khẩu
            </Button>
          </>
        }
