...
export default function AddRecipeModal({ 
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="bg-white rounded-[var(--radius-lg)] w-full max-w-2xl shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="bg-gradient-gold text-white p-6 rounded-t-[var(--radius-lg)] sticky top-0 z-10">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            {steps.map((step, index) => (
              <div key={index} className="flex gap-2">
                <div className="flex items-start gap-2 flex-1">
                  <div className="w-8 h-8 rounded-full bg-gradient-gold text-white flex items-center justify-center font-bold text-sm shrink-0 mt-2">
                    {index + 1}
                  </div>
                  <Textarea
                    value={step}
          ...
          <div className="flex gap-3 pt-4 border-t border-[var(--border-light)]">
            ...
            <Button
              type="submit"
              className="flex-1 bg-gradient-gold text-white font-semibold rounded-[var(--radius-btn)] shadow-[var(--shadow-btn)] hover-lift transition-smooth"
            >
              Lưu công thức
            </Button>
          </div>
